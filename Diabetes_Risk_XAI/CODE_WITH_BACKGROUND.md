# Full Code with Background Knowledge

## Background Knowledge

### 1. IGTD (Image Generator for Tabular Data)

IGTD converts tabular data (rows and columns of numbers) into 2D grayscale images. It arranges features in a grid (e.g., 5×3 for 15 features) so that similar features are placed close together. This lets a CNN process health data as images instead of raw numbers. The algorithm optimizes the feature arrangement by minimizing the difference between feature distances and image pixel distances.

### 2. CNN (Convolutional Neural Network)

A CNN is a deep learning model that processes images using convolutional layers. We use Shenghao Wang et al.'s architecture: 4 convolutional blocks (64 filters, 3×3 kernel), BatchNorm, ReLU, Global Average Pooling, 50% Dropout, and a fully connected output layer. It takes a 5×3 grayscale image and outputs a probability (diabetic or non-diabetic).

### 3. ENN (Edited Nearest Neighbors)

ENN removes noisy or borderline samples from the training set. It uses k-Nearest Neighbors (k=3) to find samples that are misclassified by their neighbors and removes them. This helps with class imbalance and improves model training.

### 4. Grad-CAM (Gradient-weighted Class Activation Mapping)

Grad-CAM is an Explainable AI method that shows which parts of an image the CNN focused on when making a prediction. It uses the gradients of the output with respect to the last convolutional layer's activations. Red = high importance, Blue = low importance. This helps explain *why* the model made each decision.

### 5. CDC Diabetes Dataset

The CDC (Centers for Disease Control and Prevention) diabetes dataset contains health indicators like HighBP, HighChol, BMI, Smoker, etc. We use 15 features and a binary label: 0 = Non-Diabetic, 1 = Diabetic (or prediabetic).

---

## Full Code: gradcam_explainability.py

```python
"""
Explainable AI: Grad-CAM for Diabetes CNN Predictions
Uses dataset: cdcNormalDiabeticFE1_20RFFSQ.csv (or UCI CDC fallback)
Pipeline: Load Data -> IGTD -> ENN -> CNN -> Grad-CAM
"""
...
def load_dataset_new(csv_path=None):
    ...
    Detects HTML/corrupted files (e.g., SharePoint redirect) and falls back to UCI.
    """
    if csv_path and os.path.exists(csv_path):
        try:
            ...
            if first_line.strip().lower().startswith('<!'):
                print(f"File {csv_path} appears to be HTML. Using UCI fallback.")
            else:
...
def get_feature_columns(df):
    """
    Get 15 CDC feature columns for diabetes prediction.
...
def create_igtd_images(data_df, feature_cols, output_dir, num_row=5, num_col=3, max_step=1500):
    """
    Run IGTD optimization to convert tabular data into 5×3 grayscale images.
    Returns image_data, samples, and index (for reuse on test data).
    """
...
def create_igtd_images_with_index(data_df, feature_cols, index, num_row, num_col, output_dir=None):
    """Generate IGTD images using pre-computed index (for test data, no re-optimization)."""
...
def apply_enn(X, y, n_neighbors=3):
    """
    Remove samples misclassified by k-NN. Reduces noise and helps with class imbalance.
    k=3 as per Shenghao Wang et al. cnn_igtd_f15_enn model.
    """
...
# ========== CNN Architecture (Shenghao Wang et al.) ==========
def build_cnn_pytorch(input_shape, num_classes=1):
    """
    CNN: 4 conv blocks (64 filters, 3×3), BatchNorm, ReLU,
...
class GradCAM:
    """
    Grad-CAM: Highlights which image regions influenced the CNN's prediction.
    Uses gradients of output w.r.t. last conv layer. Red=high importance, Blue=low.
    """
...
# ========== MAIN PIPELINE ==========
def run_gradcam_pipeline(csv_path=None, max_samples=500, num_gradcam_samples=8):
    """
    Full pipeline: Load Data -> Split -> ENN -> IGTD -> CNN -> Grad-CAM.
    Saves Grad-CAM heatmaps to FDI/ with feature names and color scales.
    """
    os.system('cls' if os.name == 'nt' else 'clear')

    print("=" * 60)
    print("Explainable AI: Grad-CAM for Diabetes CNN")
    print("Dataset: cdcNormalDiabeticFE1_20RFFSQ.csv (or UCI fallback)")
    print("=" * 60)

    igtd_dir = os.path.join(SCRIPT_DIR, 'IGTD_Results_XAI')
    images_dir = os.path.join(SCRIPT_DIR, 'IGTD_Images_XAI')
    gradcam_dir = os.path.join(SCRIPT_DIR, 'FDI')
    ...
    # 1. Load data
    default_csv = r"c:\Users\parva\Downloads\cdcNormalDiabeticFE1_20RFFSQ.csv"
    ...
    print(f"Saved images to IGTD_Images_XAI/")
    ...
    results_path = os.path.join(SCRIPT_DIR, 'results_summary_xai.csv')
    pd.DataFrame([{'Accuracy': accuracy, 'Precision': precision, 'Recall': recall, 'F1-Score': f1}]).to_csv(results_path, index=False)
    print(f"Results saved to results_summary_xai.csv")

    torch.save(model.state_dict(), os.path.join(SCRIPT_DIR, 'cnn_diabetes_model_xai.pt'))
    ...
    for idx, i in enumerate(indices):
        ...
        # IGTD Image with feature names and scale
        ...
        for (r, c), fname in feature_at_cell.items():
            axes[0].text(c + 0.5, r + 0.5, fname, ha='center', va='center', fontsize=5, color='yellow', weight='bold')
        axes[0].set_xlim(0, num_col)
        axes[0].set_ylim(num_row, 0)
        axes[0].axis('off')

        # Grad-CAM Heatmap with feature names and scale
        ...
        for (r, c), fname in feature_at_cell.items():
            axes[1].text(c + 0.5, r + 0.5, fname, ha='center', va='center', fontsize=5, color='white', weight='bold')
        ...
        fig.suptitle(f'Sample {idx+1}: True={true_label} ({"Diabetic" if true_label else "Non-Diabetic"}) | Pred={pred_label} ({pred_prob:.2f})', fontsize=10)
        plt.tight_layout()
    ...
    print(f"\nGrad-CAM outputs saved to {gradcam_dir}/")
    return {'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1': f1}


if __name__ == '__main__':
    default_csv = r"c:\Users\parva\Downloads\cdcNormalDiabeticFE1_20RFFSQ.csv"
    run_gradcam_pipeline(csv_path=default_csv, max_samples=500, num_gradcam_samples=8)
```
...
- scipy (for IGTD)
- ucimlrepo (for CDC dataset)
- IGTD_Functions.py (in same folder)
