...
Standalone file - runs full pipeline + Grad-CAM visualization.
"""
...
def load_dataset_new(csv_path=None):
    """
    Load dataset. Tries cdcNormalDiabeticFE1_20RFFSQ.csv first, then UCI fallback.
    Detects HTML/corrupted files and falls back to UCI.
    """
    if csv_path and os.path.exists(csv_path):
        try:
            ...
            if first_line.strip().lower().startswith('<!'):
                print(f"File {csv_path} appears to be HTML (SharePoint/corrupted). Using UCI fallback.")
            else:
...
def get_feature_columns(df):
    """Get feature columns - supports CDC and generic datasets."""
    cdc_features = [
...
def build_cnn_pytorch(input_shape, num_classes=1):
    """CNN architecture - same as main pipeline."""
...
class GradCAM:
    ...
    def _save_activation(self, module, input, output):
        self.activations = output  # Keep in graph for gradient flow
...
def run_gradcam_pipeline(csv_path=None, max_samples=500, num_gradcam_samples=8):
    """
    Full pipeline: Load data -> IGTD -> CNN -> Grad-CAM.
    Saves Grad-CAM heatmaps to FDI/ or GradCAM_Output/ folder.
    """
    # Clear terminal for fresh output on each run
    ...
    # Clear old outputs so each run is fresh
    ...
    # Save results (overwrites previous run)
    ...
    model_path = os.path.join(SCRIPT_DIR, 'cnn_diabetes_model_xai.pt')
    ...
    # Use last Conv2d (index 9) to avoid ReLU inplace backward hook issue
    target_layer = model.features[9]
    gradcam = GradCAM(model, target_layer)

    # Select samples for Grad-CAM (mix of correct/incorrect, diabetic/non-diabetic)
    ...
    # Build feature-to-position mapping (IGTD index maps grid position -> feature index)
    ...
    # Position pos (row-major) -> feature name: feature_cols[igtd_index[pos]]
    feature_at_cell = {}
    ...
    for idx, i in enumerate(indices):
        ...
        # Grad-CAM Heatmap with color scale
